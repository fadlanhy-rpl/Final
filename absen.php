...
$absen = query("SELECT * from absen ORDER BY tanggal ASC");
...
<div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4 px-10 ">
    ...
    <div class="col-span-3 grid grid-cols-1 gap-4 shadow-xl bg-white p-10 rounded-2xl">
        ...
        <?php else : ?>
            <?php
            foreach ($absen as $absent) :
                $absent['keterangan'] = html_entity_decode(html_entity_decode(strval($absent['keterangan'])));
                ...
                <div class="border-2 border-dashed rounded-lg border-gray-300 dark:border-gray-600 h-fit mb-4">
                    <a class="block h-full">
                        <div class="max-w-full border border-gray-200 rounded-lg shadow h-full flex flex-col">
                            <?php if ($absent['thumbnail']): ?>
                                <img class="rounded-t-lg w-full h-48 object-cover" alt="<?= $absent['thumbnail'] ?>" src="<?= "../assets/upload/" . $absent['thumbnail'] ?>" />
                            <?php endif ?>
                            <div class="p-5 flex-grow flex flex-col">
                                <div class="flex items-center mb-4">
                                    <div class="flex-1 min-w-0">
                                        <p class="text-sm font-medium text-gray-600 truncate"><?= $absent['nama'] ?></p>
                                        <p class="text-xs text-gray-500 truncate dark:text-gray-400"><?= $absent['tanggal'] ?></p>
                                    </div>
                                </div>
                                <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-600"><?= $absent['status'] ?></h5>
                                <?= $absent['keterangan'] ?>
                            </div>
                        </div>
                    </a>
    ...
    <div class="border-black/12.5 shadow-soft-xl relative flex h-full min-w-0 flex-col break-words rounded-2xl border-0 border-solid bg-white bg-clip-border">
                ...
                <div class="flex-auto p-4">
                    <div class="before:border-r-solid relative before:absolute before:top-0 before:left-4 before:h-full before:border-r-2 before:border-r-slate-100 before:keterangan-[''] before:lg:-ml-px">
                        <div class="relative mb-4 mt-0 after:clear-both after:table after:keterangan-['']">
                        ...
                        <div class="relative mb-4 after:clear-both after:table after:keterangan-['']">
                            <span class="w-6.5 h-6.5 text-base absolute left-4 z-10 inline-flex -translate-x-1/2 items-center justify-center rounded-full bg-white text-center font-semibold">
                        ...
                        <div class="relative mb-0 after:clear-both after:table after:keterangan-['']">
                            <span class="w-6.5 h-6.5 text-base absolute left-4 z-10 inline-flex -translate-x-1/2 items-center justify-center rounded-full bg-white text-center font-semibold">
